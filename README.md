...
- [OrcaReplay](https://github.com/Continuum-AI-Corp/OrcaReplay) - Records an agent run at the model-provider boundary and replays it offline, so a failure reproduces with no provider call and no API key — a regression test for an agent, not a dashboard. ![GitHub stars](https://img.shields.io/github/stars/Continuum-AI-Corp/OrcaReplay?style=social)
